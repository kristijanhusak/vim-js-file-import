function! jsfileimport#parser#_parse_args(selection, file_info) abort
  ...
  for l:match in l:matches
    ...
    let l:scoped = index(l:scoped_vars, l:match) > - 1
    if jsfileimport#utils#_is_reserved_word(l:match) || l:already_added || l:scoped
      let l:index += 1
      continue
    endif
    ...
    let l:_var_declared_before = l:existing_var > 0 && l:existing_var < l:from_line
    let l:is_object_prop = match(l:match.'\s*:\s*', l:content) > -1
    let l:is_var_declaration = l:index > 0 && l:matches[l:index - 1] =~? '\(const\|let\|var\)'

    " Ignore imports and duplicates
    if l:_var_declared_before || l:is_object_prop || l:exist_import || l:is_var_declaration
      let l:index += 1
      continue
    endif
...
function! jsfileimport#parser#_parse_returns(selection, file_info) abort
  ...
  for l:match in l:matches
    ...
    let l:scoped = index(l:scoped_vars, l:match) > -1
    if jsfileimport#utils#_is_reserved_word(l:match) || l:scoped || l:already_added || l:index ==? 0
      let l:index += 1
      continue
    endif
    ...
    elseif index(l:returns, l:prev) < 0 && l:is_var_assignment
